...
<template>
  ...
  <section>
    ...
    <button @click="addIngredient(data.newIngredient)" :disabled="!data.newIngredient">Add Ingredient</button>
    <table>
      <thead>
        <tr>
          <th>#</th>
...
export default {
  setup() {
    ...
    const filteredRecipes = computed(() =>
      data.recipes.filter((recipe) => !data.filter || JSON.stringify(recipe).includes(data.filter))
    )
    const add = (recipe) => {
    ...
    const addIngredient = (ingredient) => {
      ...
      data.newRecipe.ingredients = [...data.newRecipe.ingredients, data.ingredients.find((i) => i.id === ingredient)]
    }
    const removeIngredient = (ingredient) =>
      (data.newRecipe.ingredients = data.newRecipe.ingredients.filter((i) => i.id !== ingredient.id))
    return {
      filteredRecipes,
      data,
      add,
